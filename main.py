...
النظام الكامل — 10 محافظ، لكل محفظة بوت تيليجرام خاص بها:
  - يكتشف مينتات اليوم على Ink + Arc فقط
  - يشتري لجميع المحافظ المعرفة بالتوازي
  - يرسل إشعار الشراء لكل محفظة على بوت التيليجرام الخاص بها
  - بدون التحقق من X (تويتر)
"""
...
# روابط RPC للشبكتين فقط
INK_RPC_URL = os.environ.get("INK_RPC_URL", "https://rpc.ink.io")
...
def build_startup_message() -> str:
    ...
    chains = ", ".join(CHAIN_CONFIGS.keys())
    return (
        f"🚀 <b>تم تشغيل نظام الشراء التلقائي</b>\n\n"
        f"📅 الوقت: {now}\n"
        f"💰 عدد المحافظ: {len(WALLETS_DATA)}\n"
        f"🔗 الشبكات المدعومة: {chains}\n"
        f"⏳ جاهز لرصد المينتات المجانية..."
    )


def build_purchase_error_message(error_result: dict, chain_key: str) -> str:
    w_short = error_result.get('wallet', '')[:6] + "..." + error_result.get('wallet', '')[-4:] if error_result.get('wallet') else "غير معروف"
    chain_label = "Ink" if chain_key == "ink" else "Arc"
    reason = error_result.get('reason', 'unknown')
    error_msg = error_result.get('error', '')
    
    reasons_map = {
        "balance_too_low": "⚠️ رصيد غير كافٍ",
        "gas_too_high": "⛽ رسوم غاز مرتفعة",
        "no_fee_recipient": "❌ لا يوجد مستلم رسوم",
        "simulation_failed": "❌ فشل محاكاة المعاملة",
        "insufficient_funds_for_total_cost": "⚠️ رصيد غير كافٍ للتكلفة الكلية",
        "tx_error": "❌ خطأ في إرسال المعاملة",
        "already_bought": "ℹ️ تم الشراء مسبقاً",
        "sold_out": "❌ نفدت الكمية",
        "all_wallets_completed": "ℹ️ جميع المحافظ أنجزت الشراء",
        "invalid_address": "❌ عنوان غير صالح",
        "no_contract_address": "❌ لا يوجد عنوان عقد",
    }
    
    friendly_reason = reasons_map.get(reason, f"❌ سبب غير معروف: {reason}")
    
    if error_msg:
        return f"❌ <b>فشل الشراء</b>\n\n🟣 الشبكة: {chain_label}\n💰 المحفظة: <code>{w_short}</code>\n{friendly_reason}\n📝 التفاصيل: {error_msg[:100]}"
    else:
        return f"❌ <b>فشل الشراء</b>\n\n🟣 الشبكة: {chain_label}\n💰 المحفظة: <code>{w_short}</code>\n{friendly_reason}"
...
async def purchase_task_for_wallet(
    w3, item, slug, contract_address, price_wei, max_per_wallet, remaining, eth_price_usd, max_gas_fee_usd, chain_key
):
    ...
    async with lock:
        ...
        if res.get("success"):
            ...
            msg = build_purchase_success_message(item.get("current_detail", {}), res, chain_key)
            enqueue_message(bot_token, chat_id, msg)
        else:
            msg = build_purchase_error_message(res, chain_key)
            enqueue_message(bot_token, chat_id, msg)
...
async def try_buy_now_multi_wallet(slug: str, chain_key: str, detail: dict) -> list[dict] | None:
    ...
    tasks = [
        purchase_task_for_wallet(
            ...
            price_wei, max_per_wallet, remaining, eth_price_usd, max_gas_fee_usd, chain_key
        )
        for item in pending_items
    ]
...
# ---------------------------------------------------------------------------
# تقييم المينتات وإدارة المراقبة - بدون التحقق من X
# ---------------------------------------------------------------------------
...
async def evaluate_new_mint(slug: str, chain_key: str):
    ...
    try:
        ...
        log.info(f"✅ '{slug}' — تم اكتشاف مينت مجاني، جاري الشراء.")
    ...
    while True:
        try:
            async with websockets.connect(STREAM_URL, ping_interval=None, open_timeout=15) as ws:
                log.info(f"متصل بـ OpenSea Stream — يراقب لـ {len(WALLETS_DATA)} محافظ على شبكات: {', '.join(CHAIN_CONFIGS.keys())}")
                ...
                while True:
                    ...
                    log.info(f"📦 تم اكتشاف مينت جديد على {stream_chain_name}: {slug}")
