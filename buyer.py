...
# عنوان عقد SeaDrop على شبكة Ink (نفس العنوان على جميع الشبكات)
...
# إعدادات خاصة بـ Ink
MIN_BALANCE_RESERVE_USD = 0.05  # الحد الأدنى للرصيد (5 سنتات - أقل لأن الغاز رخيص)
FEW_THRESHOLD = 20
LIMITED_BUY_QTY = 15
GAS_LIMIT_SAFETY_MARGIN = 1.2
FREE_PRICE_THRESHOLD_USD = 0.01  # حد المجانية (أقل من سنت)
...
def is_free_mint(price_wei: int, eth_price_usd: float) -> bool:
    """التحقق من أن المينت مجاني (أقل من حد المجانية)"""
    if price_wei == 0:
        return True
...
def attempt_purchase_single_wallet(
    ...
    total_value = price_wei_per_token * quantity  # = 0 للمينتات المجانية
    ...
    try:
        ...
        tx = contract.functions.mintPublic(
            ...
            "value": total_value,  # 0 للمينتات المجانية
            "nonce": nonce,
        ...
        # التحقق من الرصيد الكافي (للغاز فقط، لأن السعر 0)
        ...
        log.info(f"[شراء ناجح - {checksum_wallet[:8]}] {tx_hash.hex()} — كمية: {quantity} (مجاني)")
        return {
            "success": True,
            "wallet": checksum_wallet,
            "tx_hash": tx_hash.hex(),
